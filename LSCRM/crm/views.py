...
from django.views.decorators.csrf import csrf_exempt#免除csrf
...
def stu_enrollment(request):
    ...
    if request.method == "POST":
        ...
        except IntegrityError as e:#已经生成过报名表
...
def enrollment_fileupload(request,enrollment_id):
    ...
    return HttpResponse(json.dumps({'status':True,})) #json.dumps将python对象转换为字符串再写入文件
